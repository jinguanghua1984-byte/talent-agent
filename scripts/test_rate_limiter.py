...
class TestRateLimiter(unittest.TestCase):
    ...
    def test_check_search_allowed_when_empty(self):
        """空状态时应允许搜索。"""
        from scripts.platform_match.rate_limiter import check_search
    ...
    def test_record_search_increments_count(self):
        """记录搜索应递增计数器。"""
        from scripts.platform_match.rate_limiter import record_search, _get_platform_state
        record_search("maimai")
    ...
    def test_circuit_break_triggers(self):
        """熔断触发后应阻止搜索。"""
        from scripts.platform_match.rate_limiter import trigger_circuit_break, check_search
    ...
    def test_reset_clears_state(self):
        """重置应清除所有限流状态。"""
        from scripts.platform_match.rate_limiter import record_search, check_search, _save_state
        record_search("maimai")
        # 重置: 写入空状态
        _save_state({})
        result = check_search("maimai")
