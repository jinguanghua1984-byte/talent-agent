...
前 4 级 query 必须包含非空职位或有效职位核心词；缺职位时不得自动绑定。只有前 4 级 query 且 identity score `>=95` 才允许 `auto_bound`。`name_company_fallback`、结果过多、第一二名分差过小或 score `70-94` 必须进入 `pending_confirmation`，不得自动绑定。
...
对 raw 搜索结果运行 identity scoring。query level 顺序为 `name_company_title`、`name_company_title_core`、`name_recent_company_title`、`name_school_title_core`、`name_company_fallback`。前 4 级 query 必须包含非空职位或有效职位核心词；缺职位时不得自动绑定。只有前 4 级 query 且 score `>=95` 可写 `auto_bound`。`name_company_fallback` 命中、结果过多、第一二名分差小或 score `70-94` 必须写 `pending_confirmation`，不得自动绑定。
...
如果存在 `pending_confirmation`、`missing_real_name` 或 `no_match` 且策略要求全员完成，停止并写 `state/continuation-plan.json`。人工确认后的绑定必须以 `confirmed_bound` 回写 ledger。
...
def decide_match(
    ...
    elif query_level in HIGH_PRECISION_LEVELS and not _title_core(target.current_title):
        status = "pending_confirmation"
        reason = "缺职位时高精准 query 需要人工确认"
