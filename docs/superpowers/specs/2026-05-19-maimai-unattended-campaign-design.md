...
- 每日搜索请求预算：500 次，只统计列表搜索请求，不包括详情请求。
- 搜索 wave：每组不超过 50 页。
- 详情预算：不设总人数上限，只对列表评分 A/B 档人选抓详情；详情请求不计入每日 500 次搜索请求预算。
- 详情 pack：每组上限 100 人。
...
按 wave 执行真实搜索，调用现有 live gate。编排器生成的单个 wave plan 不超过 50 页，并遵守每日搜索请求预算 500 次。该预算只统计列表搜索请求；详情请求由详情 pack 上限、health gate、请求间隔和平台安全停机规则单独约束：
...
5. Wave split 测试：单个搜索 wave plan 不超过 50 页，每日搜索请求预算为 500，且不把详情请求计入该预算。
...
- 已实践默认值已固化：每日搜索请求预算 500 且不含详情请求、搜索 wave 不超过 50 页、详情只抓 A/B 档、详情 pack 不超过 100 人。
