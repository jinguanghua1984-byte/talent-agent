...
- [x] 按字段级策略执行人才库云同步（2026-05-27）：在备份主库后真实导入已解析的远端 bundle；格里吸收远端更丰富的米哈游第一段履历描述，华珊保留本地旧版履历主体，避免盲目覆盖。
  ...
  - [x] Verify Plan：允许本轮写入 `data/talent.db`，但必须先备份；不得输出同步密钥，不上传 raw SQLite；验证包含 resolved bundle 校验、dry-run/preview 无新增冲突、真实 apply 结果、主库完整性/计数、剩余冲突状态和聚焦测试。
  - [x] Task 1：备份主库并确认同步前状态。备份 `data/backups/talent-before-cloud-strategy-sync-20260527-012634.db`；写入前 `PRAGMA integrity_check=ok`，`candidates=19881`、`candidate_details=19881`、`source_profiles=19881`、`sync_imports=11`、`sync_conflicts=3040`。
  - [x] Task 2：生成策略化 resolved bundle 并 dry-run/preview 验证。生成 `data/output/talent-cloud-strategy-sync-2026-05-27/resolved-3b06badc-9ccc-4531-83fb-6a9c13f7c0b0.zip`，`verify_ok=true`；preview import 为 `created candidates=132`、`merged candidates=19881`、`conflicts candidate_details=0`、preview 完整性 `ok`。
  - [x] Task 3：真实 apply 到 `data/talent.db` 并更新 cloud-state。真实导入 resolved bundle：`created candidates=132`、`merged candidates=19881`、`conflicts=0`；格里远端更细米哈游第一段描述写入 raw_data 3 处和 structured work_experience 1 处；`cloud-state` 将 `3b06...` 标记 applied，并将重复 full bundle `c8ab...` 作为同源重复包标记跳过。
  - [x] Task 4：运行验证并写回 Review。
  - Review：已按字段级策略完成真实主库同步。写库前备份为 `data/backups/talent-before-cloud-strategy-sync-20260527-012634.db`。已生成并校验 resolved bundle `data/output/talent-cloud-strategy-sync-2026-05-27/resolved-3b06badc-9ccc-4531-83fb-6a9c13f7c0b0.zip`，preview 无新增冲突；真实导入 `bundle_id=3b06badc-9ccc-4531-83fb-6a9c13f7c0b0`，结果 `created candidates=132`、`created candidate_details=132`、`created source_profiles=132`、`merged candidates=19881`、`merged candidate_details=19881`、`merged source_profiles=19881`、`conflicts=0`。格里 `maimai:242263588` 已吸收远端更丰富的米哈游第一段履历描述，写入 raw_data 3 处和 structured `work_experience` 1 处；华珊 `maimai:237594594` 保留本地旧版履历主体和 7 个项目。主库写后 `PRAGMA integrity_check=ok`，`candidates=20013`、`candidate_details=20013`、`source_profiles=20013`、`sync_imports=12`、本次 bundle 新增冲突 `0`。`data/sync/cloud-state.json` 已清空 blocked list，并把 `3b06...` 与同源重复 full bundle `c8ab...` 标记为 applied/skipped；`c8ab...` 未另写 `sync_imports`。验证：`python -m scripts.talent_cloud_sync status` 显示候选人 `20013`、已应用 bundle `2`；格里首段描述长度已为 `483` 且保留 3 个项目，华珊保留 9 段工作经历和 7 个项目；聚焦 `python -m pytest tests\test_talent_sync.py tests\test_talent_db.py tests\test_talent_cloud_sync.py -q` -> `192 passed`；全量 `python -m pytest tests scripts -q` -> `934 passed, 1 warning`，warning 为既有 `scripts/test_boss.py` event loop deprecation；`git diff --check` 通过。
- [x] 优化人才库云同步 raw detail 语义合并并重新比较 67 个冲突（2026-05-27）：按用户引用建议，让 `maimai_detail_capture` 忽略采集易变字段、归一化接口包装，并对可能丢信息的项目字段保守保留更完整版本，然后只读复算 blocked bundle 差异。
  - [x] Plan：先用 TDD 增加语义合并回归，覆盖 token/URL/时间戳/capture_file/mode 不阻断、`job_preferences` 和 endpoint wrapper 归一化、`user_project` 保留更完整版本，以及真实内容变化仍记录冲突；再修改 `scripts/talent_db.py` 的 raw_data merge；最后用本地 blocked bundle 和 `data/talent.db` 只读重算冲突。
  - [x] Verify Plan：不执行真实 `sync apply`，不写 `data/talent.db`；只读 bundle/SQLite，复算报告写入 `data/output/`；验证包含新增测试红灯/绿灯、聚焦同步测试、DB 完整性计数和 `git diff --check`。
  - [x] Task 1：补语义合并红灯测试并确认失败原因。新增测试当前失败在 `result["conflicts"]["candidate_details"] == 0`，实际仍为 `1`；真实字段变化用例保持冲突。
  - [x] Task 2：实现 `maimai_detail_capture` 语义归一化/保守合并。已忽略采集文件、模式、token/URL、时间戳、活跃/年龄/浏览状态、接口响应包装、source_contact、friends/hover、内部 id/file 元数据；归一化 `job_preferences`、标量标签列表和城市省名前缀；`user_project` 不参与冲突比较且合并时保留更完整值。
  - [x] Task 3：重跑测试并只读复算 67 个冲突的新差异。真实 preview import 由 `candidate_details=67` 降为 `2`；剩余为格里、华珊两人的履历描述/工作经历实质差异。
  - [x] Task 4：写回 Review 和处理建议。
  - Review：已按语义 merge 优化同步合并逻辑。新增回归测试覆盖：采集噪声和包装差异不再记录冲突、`user_project` 保留本地更完整项目、真实 `current_title` 变化仍记录 `candidate_detail.raw_data.maimai_detail_capture` 冲突。只读复算 blocked bundle `3b06badc-9ccc-4531-83fb-6a9c13f7c0b0`：优化前 `67` 个详情冲突，优化后 preview import 为 `candidate_details=2`，仍计划 `created candidates=132`、`merged candidates=19881`，preview DB 完整性 `ok`。剩余 2 个候选人为 `maimai:242263588` 格里（教育/工作 description 差异）和 `maimai:237594594` 华珊（工作公司、部门、职位、description、标签数量等履历内容差异），建议继续保留阻断并人工择优/版本化处理，不自动吞并。新报告位于 `data/output/talent-cloud-sync-conflicts-2026-05-27-semantic-merge/`：`semantic-summary.json`、`semantic-report.md`、`remaining-semantic-conflicts.csv`、`remaining-diff-paths.csv`、`resolved-candidates.csv`。验证：新增红灯已确认；聚焦 `python -m pytest tests\test_talent_sync.py tests\test_talent_db.py tests\test_talent_cloud_sync.py -q` -> `192 passed`；全量 `python -m pytest tests scripts -q` -> `934 passed, 1 warning`，warning 为既有 `scripts/test_boss.py` event loop deprecation；`py_compile`、`git diff --check` 通过；主库未写入，`PRAGMA integrity_check=ok`，`candidates=19881`、`candidate_details=19881`、`sync_imports=11`。
- [x] 分析人才库云同步 67 个详情冲突（2026-05-27）：基于 blocked bundle 和本地 `data/talent.db` 只读分析 `candidate_details` 冲突具体情况，并给出处理建议。
  - [x] Plan：读取 `data/sync/cloud-state.json`、本地已下载远端 full bundle 和同步冲突合并逻辑；复现冲突字段，不写主库；按字段、来源、风险和可自动处理程度分组；输出摘要和建议。
  - [x] Verify Plan：不执行 `sync apply`、不修改 `data/talent.db`；只读 zip/SQLite；若生成报告，只落到 `data/output/`；最终用完整性和计数确认主库未被写入。
  - [x] Task 1：复现 67 个冲突并生成字段/候选分布。
  - [x] Task 2：抽样检查本地/远端差异内容，归类根因。
  - [x] Task 3：给出处理建议和验证证据。
  - Review：已只读分析 blocked bundle `3b06badc-9ccc-4531-83fb-6a9c13f7c0b0`。同步计划为 `created candidates=132`、`merged candidates=19881`，候选人和 source profile 无冲突；阻断点全部集中在 `candidate_details.raw_data.maimai_detail_capture`，共 `67` 条、覆盖 `67` 个候选人，原因均为同一 namespace 下本地/远端 JSON 值不同。本地冲突详情来源主要为 `hunyuan-8jd-abc-detail-2026-05-22` 50 条、`tencent-games-ai-100hc-broad-recall-2026-05-24` 15 条、旧 AI Infra/legacy capture 各 1 条；远端 67 条均来自 `data/output/maimai-detail-capture-2026-05-25-filtered-for-import.json`。差异以采集批次/模式/链接 token/时间戳/接口包装 schema 为主：`capture_file`、`detail_url`、`trackable_token`、`started_at/finished_at` 均 67 条不同，`mode`、`job_preferences` 包装、`contact_btn/job_preference/projects` 响应包装和 `user_project` 表达方式多为 66 条不同。已生成报告 `data/output/talent-cloud-sync-conflicts-2026-05-27/summary.json`、`conflicts.csv`、`candidates.csv`、`diff-analysis.json`、`diff-paths.csv`、`candidate-diff-paths.csv`、`report.md`。建议不要盲目覆盖 67 条详情；先保留本地详情作为主值，把远端 2026-05-25 capture 作为同 namespace 的备选/版本化 raw，或实现可忽略易变字段的语义 merge 后再导入。验证：主库未写入，`PRAGMA integrity_check=ok`，`candidates=19881`、`candidate_details=19881`、`sync_imports=11`。

- [x] 同步本机人才库（2026-05-26）：按用户要求执行云端人才库同步，使用现有 Feishu Drive provider 和本机 `.env`，同步后核对状态。
  - [x] Plan：先检查工作区、`status` 和 `doctor`；再执行 `python -m scripts.talent_cloud_sync sync`；最后读取同步结果、复查本机状态、运行必要验证并记录 Review。
  - [x] Verify Plan：只使用 `scripts.talent_cloud_sync` 正常同步入口；不手动复制/覆盖 `data/talent.db`，不输出 Fernet 同步密钥，不上传 raw SQLite；若 pull 阶段出现冲突阻断，则停止自动 push 并报告 blocked bundle。
  - [x] Task 1：同步前状态检查。当前 provider 为 `feishu`，本机 node_id 已识别，`status` 显示候选人 `19881`、已应用 bundle `0`；工作区有上一轮 doctor 修复的未提交代码/任务记录变更，保留不回退。
  - [x] Task 2：运行 `doctor` 和真实 `sync`。`doctor` 返回 `ok=true`，quota API `90007001` 继续降级为 warning；首次 sync 因缺少 `sync_id` 索引长时间卡在 import planning，已停止本次进程，DB 完整性仍为 `ok`。
  - [x] Task 3：补同步查询索引并重新 sync。已为 candidates/detail/source/timeline/score/match 的 `sync_id` 建索引，现有主库查询计划确认走 `idx_candidates_sync_id`；重新 sync 快速返回，但 pull 阶段被安全阻断：两个远端 full bundle 均为 `created candidates=132`、`merged candidates=19881`、`conflicts candidate_details=67`，因此未 apply、未 push。
  - [x] Task 4：运行验证并写回 Review。
  - Review：本次同步已执行但被安全阻断，未 apply 远端 bundle，未 push 本机 bundle。同步前 `status` 为 provider `feishu`、本机候选人 `19881`、已应用 bundle `0`；`doctor` 为 `ok=true`，quota API `90007001` 作为 warning。首次 sync 暴露大库 import planning 缺少 `sync_id` 索引导致长时间 CPU 计算，已停止该进程，主库 `PRAGMA integrity_check=ok`。已补 `sync_id` 索引并验证现有主库查询计划走 `idx_candidates_sync_id`；重新 sync 快速返回 blocked：远端 bundle `3b06badc-9ccc-4531-83fb-6a9c13f7c0b0` 和 `c8ab43fd-964e-4ff1-9f2b-224284685711` 均计划 `created candidates=132`、`merged candidates=19881`，但 `candidate_details=67` 冲突，因此按设计停止自动写库并跳过 push。同步后主库仍为 `candidates=19881`、`candidate_details=19881`、`sync_imports=11`，完整性 `ok`；`data/sync/cloud-state.json` 已记录 blocked bundles。验证：聚焦同步回归 `190 passed`；全量 `python -m pytest tests scripts -q` -> `932 passed, 1 warning`，warning 为既有 `scripts/test_boss.py` event loop deprecation；`py_compile` 和 `git diff --check` 通过。

- [x] 修复云同步 doctor 本机 lark-cli 解析（2026-05-26）：让 `python -m scripts.talent_cloud_sync doctor` 在 Windows npm shim 环境下可直接运行。
  - [x] Plan：复现用户命令；确认 `.venv` 不存在后改用系统 Python；定位失败为 `subprocess.run(["lark-cli", ...])` 找不到 Windows npm shim；修复默认 runner 的 executable 解析，补回归测试，再重跑 doctor。
  - [x] Verify Plan：修改边界为 `scripts/talent_cloud_sync_providers.py`、`tests/test_talent_cloud_sync.py`、任务记录和必要 error log；不改 `.env`，不输出同步密钥，不上传/下载数据 bundle，不写 `data/talent.db`。
  - [x] Task 1：实现 shim 解析和回归测试。`_run_lark_cli` 现在会把裸 `lark-cli` 解析为可执行 shim，Windows 优先 `lark-cli.cmd`；新增回归覆盖默认 runner 解析 `.cmd`。
  - [x] Task 2：运行聚焦测试、编译、diff hygiene 和 doctor。聚焦云同步测试 `27 passed`，`py_compile` 与 `git diff --check` 通过；用户完成 no-wait 设备授权后新增 scope `drive:quota_detail:read_one`，doctor 返回 `ok=true`，quota API `90007001` 按既有逻辑降级为 warning。
  - Review：已修复 Windows 下 provider 默认 runner 无法执行 npm `lark-cli` shim 的问题；`_run_lark_cli` 现在优先解析 `lark-cli.cmd`、`lark-cli.exe`、`lark-cli`、`lark-cli.ps1`，并补回归测试覆盖 `.cmd` 路径。`.venv` 在当前 Windows 工作区不存在，已用系统 `python` 执行同等模块命令。验证：`python -m scripts.talent_cloud_sync doctor` -> `ok=true` 且 quota warning；`python -m pytest tests\test_talent_cloud_sync.py -q` -> `27 passed`；`python -m py_compile scripts\talent_cloud_sync.py scripts\talent_cloud_sync_providers.py` 通过；`git diff --check` 通过；全量 `python -m pytest tests scripts -q` -> `931 passed, 1 warning`，warning 为既有 `scripts/test_boss.py` event loop deprecation。
