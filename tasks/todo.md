...
- [ ] 脚本清理和 hygiene 第一轮实施（2026-05-27）：执行 `docs/superpowers/plans/2026-05-26-script-cleanup-and-hygiene.md`，将测试文件迁出 `scripts/`，移除已替代 legacy 入口，建立脚本清单和护栏测试；Hunyuan ABC 一次性脚本已获确认并删除，data manager 入口兼容改造和 AI Infra 旧策略兼容层标记已完成。
  - [x] Plan：按既有实施计划分任务推进；Task 1 建立 `docs/dev/script-inventory.md` 和 `tests/test_script_hygiene.py`，Task 2 迁移 `scripts/test_*.py`，Task 3 删除已有替代的 `score_candidates.py`，Task 4 已在用户确认后删除 `scripts/hunyuan_abc_detail_tasks.py` 和 `scripts/hunyuan_abc_parallel_supervisor.ps1`，Task 5 将 `data-manager.py` 改为 importable module 并保留旧命令 shim。
  ...
  - [x] Task 3：移除 `score_candidates.py` legacy 入口。验证：红灯 `.venv/bin/python -m pytest tests/test_script_hygiene.py::test_legacy_score_candidates_is_not_a_runtime_script -q` -> `1 failed`，失败原因为 `scripts/score_candidates.py` 仍存在；引用扫描 `rg -n "score_candidates.py|scripts.score_candidates" README.md AGENTS.md agents scripts tests docs/manual docs/dev --glob '!scripts/score_candidates.py'` 仅命中本任务护栏测试和脚本清单记录，排除这两个计划内文件后无活动引用；`git rm scripts/score_candidates.py` 成功；绿灯 `.venv/bin/python -m pytest tests/test_script_hygiene.py -q` -> `3 passed`，`.venv/bin/python -m pytest tests/test_score_pipeline.py -q` -> `9 passed`。
  - [x] Task 4：Hunyuan ABC 一次性脚本删除审批门（已获用户 `确认`，完成批准后的删除、清单更新和聚焦验证）。
    - [x] Step 1-2：新增 `test_runtime_scripts_do_not_hardcode_dated_campaign_paths` 护栏。红灯验证：`.venv/bin/python -m pytest tests/test_script_hygiene.py::test_runtime_scripts_do_not_hardcode_dated_campaign_paths -q` -> `1 failed`，offenders 为 `hunyuan_abc_detail_tasks.py` 和 `hunyuan_abc_parallel_supervisor.ps1`。
    - [x] Step 3：按计划运行 Hunyuan 引用扫描，输出仅命中计划内 `docs/dev/script-inventory.md` 清单条目和 `tests/test_script_hygiene.py` 基线测试条目；README、AGENTS、agents、scripts、docs/manual 中未发现活动 runtime/workflow 引用，活动子集扫描无输出。
    - [x] Step 4：用户已明确回复 `确认`，批准删除 `scripts/hunyuan_abc_detail_tasks.py` 和 `scripts/hunyuan_abc_parallel_supervisor.ps1`。
    - [x] Step 5：`git rm scripts/hunyuan_abc_detail_tasks.py scripts/hunyuan_abc_parallel_supervisor.ps1` 成功，两个已批准一次性脚本已从运行时目录移除。
    - [x] Step 6：`docs/dev/script-inventory.md` 已改为记录两个 Hunyuan 条目“已移出运行时目录”，历史执行记录保留在 `tasks/archive/2026-05.md`。
    - [x] Step 7：聚焦验证通过：`.venv/bin/python -m pytest tests/test_script_hygiene.py -q` -> `4 passed`；全路径扫描 `rg -n "hunyuan_abc_detail_tasks|hunyuan_abc_parallel_supervisor" README.md AGENTS.md agents scripts tests docs/manual docs/dev` 仅命中 `docs/dev/script-inventory.md` 的历史清单条目和 `tests/test_script_hygiene.py` 的护栏 marker；runtime/workflow 子集 `README.md AGENTS.md agents scripts docs/manual` 无输出。
    - [x] Step 8：按批准门删除准备提交，提交消息为 `refactor: remove approved hunyuan abc one-off scripts`。
  - [x] Task 5：将 data manager 转为 importable module 并保留旧命令 shim。验证：红灯 `.venv/bin/python -m pytest tests/test_data_manager.py::TestEntrypoints -q` -> `1 failed, 1 passed`，直接运行确认 `No module named scripts.data_manager`；补强 legacy shim 测试后红灯 -> `2 failed, 1 passed`；绿灯入口测试 -> `3 passed`；`.venv/bin/python scripts/data-manager.py` 和 `cd /tmp && /Users/eric/workspace/talent-agent/.venv/bin/python /Users/eric/workspace/talent-agent/scripts/data-manager.py` 均打印 usage（无 import error，退出码 1 为无参数 help 行为）；聚焦 `.venv/bin/python -m pytest tests/test_data_manager.py tests/test_script_hygiene.py -q` -> `47 passed`；`.venv/bin/python -m py_compile scripts/data_manager.py scripts/data-manager.py` 通过；活动引用扫描 `rg -n "python scripts/data-manager.py" README.md agents/workflows docs/manual docs/dev` 无输出。
  - [x] Task 6：保留并标记 `maimai_ai_infra_*` legacy AI Infra 兼容层。Review 修复：文档 no-delete 原则明确覆盖现有完整 `maimai_ai_infra_*` 兼容模块组，并逐项列出当前 12 个前缀模块；hygiene guard 改为动态扫描 `scripts/maimai_ai_infra_*.py`，防止新增/遗漏模块未写入清单。验证：动态 guard 红灯先捕获 9 个遗漏文件；补清单后 `.venv/bin/python -m pytest tests/test_script_hygiene.py::test_ai_infra_legacy_modules_are_documented_as_compatibility_layer -q` -> `1 passed`；路由回归 `.venv/bin/python -m pytest tests/test_maimai_campaign_orchestrator.py tests/test_maimai_ai_infra_strategy.py tests/test_maimai_ai_infra_delivery_report.py -q` -> `61 passed`；完整 hygiene `.venv/bin/python -m pytest tests/test_script_hygiene.py -q` -> `5 passed`；`git diff --check` 通过。
