...
- [ ] 脚本清理和 hygiene 第一轮实施（2026-05-27）：执行 `docs/superpowers/plans/2026-05-26-script-cleanup-and-hygiene.md`，将测试文件迁出 `scripts/`，移除已替代 legacy 入口，建立脚本清单和护栏测试，并停在 Hunyuan ABC 一次性脚本删除审批门。
  ...
  - [ ] Task 4：Hunyuan ABC 一次性脚本删除审批门（已完成预审批 Step 1-3；等待明确删除确认后才继续）。
    ...
    - [ ] Step 4：停在删除审批门；未获明确确认前不删除 `scripts/hunyuan_abc_detail_tasks.py`、`scripts/hunyuan_abc_parallel_supervisor.ps1`，也不修改 `docs/dev/script-inventory.md` 的 Hunyuan 条目。
