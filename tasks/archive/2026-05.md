...
# 处理训练/推理框架宽召回同步冲突（2026-05-31）

> 目标：处理 `ai-infra-framework-broad-recall-2026-05-29` 导入主库后新增的 `2957` 条 open `sync_conflicts`，只处理本次 bundle `b6b8b506-9cec-4a14-98ca-3e651f96ba9a`。

## 计划

- [x] 只读审计本次 bundle 的 open `sync_conflicts`：共 `2957` 条。
- [x] 生成逐条冲突处理计划：keep-local / use-remote / standardized-remote。
- [x] 备份正式库，事务内处理自动可判定冲突，并做 local-value drift 校验。
- [x] 校验剩余 open 冲突、DB 完整性、关键计数和测试。
- [x] 更新任务记录；高风险项保留正式库，不强行覆盖。

## Review

- 本次处理范围严格限定为 bundle `b6b8b506-9cec-4a14-98ca-3e651f96ba9a`，写前本 bundle open conflicts 为 `2957`。
- 处理计划：`data/output/talent-conflict-resolution-plan-2026-05-31-ai-infra-framework-broad-recall.md`；逐条 JSON：`data/output/talent-conflict-resolution-plan-2026-05-31-ai-infra-framework-broad-recall.json`。
- 写前备份：`data/backups/talent-db-before-ai-infra-framework-conflict-resolution-20260531-201815.db`。
- 已在单个 SQLite 事务内处理 `2957` 条冲突：`resolved_keep_local=2539`、`resolved_use_remote=381`、`resolved_standardized_remote=37`；处理结果：`data/output/talent-conflict-resolution-result-2026-05-31-ai-infra-framework-broad-recall.md` / `.json`。
- 实际字段更新 `418` 处：`candidates=291`、`candidate_details=127`；所有写入前均校验当前值仍等于 `sync_conflicts.local_value`，未发现 drift。
- `candidate_detail.raw_data.maimai_list` 冲突 `2450` 条全部保留正式库；`maimai_detail_capture` 冲突 `131` 条中，`127` 条采用本轮详情 capture，`4` 条因远端规范化证据量明显更少而保留正式库。
- 候选人字段处理：姓名、远端未知性别、学历降级、工作年限降低和低置信当前公司/职位/期望职位保留正式库；年龄自然递增、求职状态、所在城市、期望城市/薪资及高置信当前公司/职位采用本轮值或标准化值。
- 写后验证通过：本 bundle `open_conflicts=0`，全库 `open_conflicts=0`；逐条值校验 `2957/2957` 通过；`PRAGMA integrity_check=ok`；正式库计数为 `candidates=56193`、`candidate_details=56193`、`source_profiles=56198`、`candidate_fts=56193`、`sync_conflicts=3249`、`maimai_detail_capture=22590`。
- 验证：`git diff --check` 通过；`.venv/bin/python -m pytest tests -q` -> `995 passed, 1 warning`。

# 将训练/推理框架宽召回数据同步到正式库（2026-05-31）

> 目标：将 `ai-infra-framework-broad-recall-2026-05-29` Campaign DB 数据写入正式人才库 `data/talent.db`。

## Review

- 已按标准 bundle 流程执行，未直接复制 SQLite：export -> verify-bundle -> dry-run import -> 备份正式库 -> apply import。
- Bundle：`data/output/talent-sync-ai-infra-framework-broad-recall-2026-05-29-20260531-195122.zip`；bundle id `b6b8b506-9cec-4a14-98ca-3e651f96ba9a`。
- 备份：`data/backups/talent-20260531-195224-before-ai-infra-framework-broad-recall-sync.db`。
- Dry-run：新建 `29969`、合并 `5563`、冲突候选人 `0`。
- Apply：新建 `29969`、合并 `5563`、冲突候选人 `376`；正式库当前 `candidates=56193`、`candidate_details=56193`、`source_profiles=56198`、`candidate_fts=56193`。
- 本次 bundle 产生 open conflicts `2957` 条，已保留在 `sync_conflicts`，未静默覆盖正式库已有字段；审计报告为 `data/output/talent-sync-ai-infra-framework-broad-recall-2026-05-29-conflicts-20260531.md` 和 `.json`。
...
# 脉脉训练/推理框架宽召回自适应寻访（2026-05-29 至 2026-05-31）

> 目标：执行 `ai-infra-framework-broad-recall-2026-05-29` 宽召回自适应实验模式，覆盖训练框架、推理框架目标公司池；搜索全部覆盖后导入 Campaign DB，除淘汰外全量抓取详情，生成宽召回摘要。主库同步和飞书发布不在本轮自动边界内。

## Review

- 搜索阶段完成 264 个 unit，canonical raw `2138` 页，`seen_candidates=35538`；11 个 search wave dry-run/apply 均 clean，Campaign DB `candidates=35532`。
- 详情优先级：`detail_p0=145`、`detail_p1=14291`、`detail_p2=214`、`skip=20882`；非淘汰目标 `14650` 人，拆分 `147` 个 pack。
- 详情 live 采用 4 并发，最终完成 `147/147` 个 pack、`14650/14650` 个 job raw；中途 `detail-ab-pack-014` 出现一次 `TypeError: Failed to fetch`，用户验证后从持久化 job raw 恢复。
- 详情 apply 写入 Campaign DB `14606` 条；`39` 个 `missing_work_experience` blocker 未伪造工作经历，已按既有规则生成 clean 子集 capture 并 apply，原始 blocker 证据见 `reports/detail-wave-clean-subset-blockers-2026-05-31.json`。
- 宽召回摘要已生成：`reports/broad-recall-summary.md` 和 `reports/broad-recall-summary.json`。
...
# BOSS 当前列表高薪人选详情浏览（2026-05-30）

> 目标：轮询阅读 BOSS 直聘当前展示的人选列表，对薪资上限或明确薪资超过 50K 的卡片进入详情页查看，然后返回列表继续浏览。

## 计划

- [x] 读取项目 Boss 渠道边界，确认不得使用主动 fetch、new_page 或直接详情 URL。
- [x] 识别当前 BOSS 应用列表页已展示的人选卡片与薪资。
- [x] 对薪资上限或明确薪资超过 50K 的人选，按 UI 点击卡片进入详情页查看。
- [x] 返回列表继续浏览其他当前展示或滚动后展示的人选。
- [x] 记录本次结果与验证证据。

## 边界

- 仅做用户已授权的界面浏览、筛选和导航。
- 未主动发消息、未投递、未收藏、未保存联系方式、未写入主库。
- 未使用 Boss 主动接口，未直接访问 `/web/geek/{id}`，未调用 `fetch/new_page/goto` 等高风险路径。
- 未处理登录、验证码、风控或任何提交类动作。

## Review

- 按 UI 正常点击并查看 7 个薪资相关卡片详情，薪资段分别为 `180-250K`、`50-80K`、`80-110K`、`45-65K`、`30-60K`、`80-110K`、`60-80K`。
- `25-50K`、`40-50K`、`20-40K`、`20-30K` 等未超过 50K 的卡片已跳过；中途误触过一张 `15-30K` 卡片，立即返回，未执行任何动作。
- 最终停留在 BOSS 列表页，可继续滚动浏览。
- 本次不改业务代码、不写数据库；除 `tasks/todo.md` 与 `tasks/archive/2026-05.md` 的任务记录外无业务产物。
- 验证证据：每次进入详情页后均通过左上返回回到列表；未触发沟通、收藏、发送、保存、登录或验证码流程。未运行 pytest，因为本次不是代码改造。

# 平台化开放服务产品方案设计（2026-05-28 至 2026-05-29）

> 目标：设计将脉脉寻访、按 JD 从人才库推荐、公司级总人才库、飞书交付、个人库向主库同步、账户/计费/积分体系平台化为 SaaS 开放服务的完整产品方案，并包含业务架构和技术架构可视化表达。

## 边界

- 只做方案设计和规格文档，不改业务脚本、不执行脉脉/飞书真实任务、不写 `data/talent.db`。
- 复用现有 `maimai-unattended-campaign`、`jd-talent-delivery`、`talent-library`、`talent_cloud_sync` 能力边界。
- 平台化方案必须保留平台安全、人审、dry-run、同步冲突和隐私数据隔离边界。
- 用户要求文档写完后自动关闭计算机，不进入下个阶段；本次不创建实施计划。

## 计划

- [x] 梳理现有人才库、JD 推荐、脉脉寻访、飞书交付和云同步上下文。
- [x] 确认产品拆分范围和首期形态：采用混合路线，首期先做工单、公司库、飞书交付和积分账本；注册/支付/自动授权后置。
- [x] 确认首期用户对象：内部可信用户为主，权限与数据模型预留外部合作方接入。
- [x] 确认主库策略：采用分层主库，无联系方式线索可更自动入线索层，有联系方式优质人选需审核进入可服务层。
- [x] 确认脉脉授权策略：采用混合授权，首期公司托管账号池与人工处理登录/验证码，预留用户自带账号能力。
- [x] 确认计费账本策略：采用双账本，贡献积分与服务额度分开；首期人工发放/结算，后续再接支付和发票。
- [x] 确认首期协作入口：飞书做业务协作主入口，平台提供只读结果页、数据包下载、账本和审计视图。
- [x] 提出 2-3 种平台化路线及取舍建议：确认采用“服务中台 + 飞书协作”路线。
- [x] 输出业务架构、技术架构、数据流、计费/积分、权限和运营闭环设计。
- [x] 经确认后写入设计文档并完成自检。

## Review

- 设计文档已写入 `docs/superpowers/specs/2026-05-29-platform-talent-service-design.md`。
- 方案采用“服务中台 + 飞书协作”：平台负责工单、异步执行、公司主库、同步、账本、权限和审计；飞书负责首期业务协作。
- 技术架构优先复用商业友好开源组件：Temporal、Keycloak、Casbin、OpenMeter、Refine/React-admin、Postgres/pgvector、Haystack、FastAPI；n8n、Lago、Activepieces 等因许可或 EE 边界只作为观察项。
- 数据架构确认平台公司主库为事实源，本地个人库为个人工作副本；上行通过加密 bundle、校验、dry-run、去重、审核和分层入库，下行通过脱敏数据包更新个人库。
- 计费和激励采用双账本：贡献积分与服务额度分开；提交贡献先冻结，审核后入账；工单提交预占额度，完成后按实际用量结算。
- 权限和审计覆盖联系方式查看、数据包下载、额度调整、工单执行、账号池使用和飞书发布。
- 阶段路线分为 P0 设计与验证、P1 内部服务中台、P2 半开放合作方、P3 SaaS 商业化；P1 验收标准共 10 条。
- 自检包含占位符/冲突标记检查和 `git diff --check`；本任务不进入 `writing-plans` 或实施计划。
...
# BOSS App 推荐列表寻访工作流首版（2026-05-31）

> 目标：把 BOSS 直聘 App 推荐列表寻访做成运行时中立、可恢复、可审计的 workflow。执行载体是 Computer Use 操作本机 BOSS App，不处理网页端/CDP/BOSS API。

## 需求确认

- 用户先打开 BOSS App 并进入目标职位的牛人推荐列表页；workflow 从当前列表逐卡片处理，不制定搜索策略。
- 用户通常只提供基于 JD 推荐后的进一步筛选依据，例如公司、职位、学历、年龄、技术栈。
- 首版详情证据只保存结构化文本和截图哈希，不保存截图文件。
- 默认 `dry_run`：只定位 `立即沟通` 并记录 `would_contact`，不点击。
- 少量 live-test 允许真实点击 `立即沟通`，但必须同时开启 `allow_real_contact=true`、`allow_live_contact_test=true`、设置正数 `live_contact_test_limit`，且每次动作前确认。
- 列表/详情页展示名可能是 `张先生` 等加密名；沟通页可获得真实姓名，数据模型必须同时保留 `display_name` 和 `real_name`。

## 关键产物

- 设计文档：`docs/superpowers/specs/2026-05-31-boss-app-recommendation-sourcing-design.md`
- 实施计划：`docs/superpowers/plans/2026-05-31-boss-app-recommendation-sourcing.md`
- Canonical skill：`agents/skills/boss-app-recommendation-sourcing/SKILL.md`
- Canonical workflow：`agents/workflows/boss-app-recommendation-sourcing/AGENT.md`
- Claude adapter：`.claude/skills/boss-app-recommendation-sourcing/SKILL.md`
- 能力合同：`agents/capabilities.md` 新增 `computer.operate`
- Python helper：`scripts/boss_app_sourcing.py`
- 测试：`tests/test_boss_app_sourcing.py`、`tests/test_agent_architecture.py`

## 实现内容

- `init_campaign` 创建独立任务目录，写入 `requirements.json`、`strategy.json`、`run-policy.json`、`campaign-manifest.json`、raw/state/structured/reports 骨架，并拒绝覆盖既有或部分初始化目录。
- `validate_run_policy` 严格校验安全字段类型和 live-test 三条件；直传 `contact_mode=live_test` 不能绕过。
- JSON/JSONL helper 支持 BOM 读取、malformed JSONL 行号报错、append 时修复缺失尾部换行。
- 候选人 lifecycle 支持 `record_list_card`、`record_detail_update`、`record_contact_decision`、`backfill_real_name` 和 `latest_candidate`。
- Candidate key 包含展示名、可见字段、截图哈希、列表滚动批次和卡片位置。
- append-only candidate 更新基于 latest candidate 合并，重复列表卡片不会重置详情、联系或真实姓名状态。
- `dry_run` 不标记 `contacted`；live-test 必须先有详情 `contact` 判定和既有 `would_contact`，且同一候选人不能重复真实触达。
- live 后再次 dry-run 不会把 `contacted/live_contact_test/preset_message_auto_sent` 降级。
- `backfill_real_name` 保留 `display_name`，拒绝空名、非法 source、未 live contacted 却声明 live-source，以及不同真实姓名/来源覆盖。
- `write_continuation_plan` 写 `state/continuation-plan.json` 并追加 `state/events.jsonl`。
- `summarize_campaign` 和 `summarize` CLI 写 JSON/Markdown 报告，包含候选人数、列表卡片数、详情数、would-contact、live-test、live-test 剩余额度、真实姓名状态分布、跳过原因分布、人工复核清单和恢复入口。

## Review 与修复

- Task 2 spec review 修复 candidate key 未包含列表滚动批次和卡片位置的问题。
- Task 2 code quality review 修复非空无 manifest 目录覆盖、policy 类型 truthiness 绕过、JSONL 无尾部换行追加损坏、`init_campaign` 入口 `int()` 绕过问题。
- Task 3 spec review 修复 live-test 缺少详情 `contact` 和 `would_contact` 前置门禁。
- Task 3 code quality review 修复重复列表卡片重置 latest、同候选人重复 live-test、live 后 dry-run 降级、真实姓名 source/覆盖、raw 恢复字段不足。
- Task 4 spec review 修复 `detail_count` 依赖非空详情段落和 S8 报告字段不足。
- Task 4 code quality review 修复缺失/损坏/非对象 `run-policy.json` 时静默出报告的问题，并补 latest 去重和 events JSONL 可读性测试。

## 验证

- `.venv/bin/python -m pytest tests/test_boss_app_sourcing.py -q` -> `36 passed`
- `.venv/bin/python -m pytest tests/test_boss_app_sourcing.py tests/test_agent_architecture.py tests/test_script_hygiene.py -q` -> `49 passed`
- `.venv/bin/python -m py_compile scripts/boss_app_sourcing.py` -> exit 0
- `.venv/bin/python -m pytest tests -q` -> `995 passed, 1 warning`
...
- 未真实操作 BOSS App。
- 未点击 `立即沟通`。
- 未写主人才库 `data/talent.db`。
- 未实现平台 UI 执行器；真实 UI 操作由 canonical workflow 的 `computer.operate` 交给运行时适配器执行。

# BOSS App 推荐列表寻访试跑 50 人（2026-05-31）

> 目标：按用户要求在 BOSS App 当前推荐列表中不设置额外筛选条件，列表中全部人选都视为匹配，逐个进入详情页采集信息，并只做 `dry-run` 联系判断。先跑 50 人后停止汇报。

## 执行边界

- 执行面：本机 BOSS 直聘 App + Computer Use。
- 不处理网页端、CDP 或 BOSS API。
- 无额外筛选条件：可见列表人选全部进入详情。
- 全程 `dry-run`：只记录 `would_contact=true` 和 `立即沟通` 按钮可见，不点击 `立即沟通`，不发送消息。
- 不写主人才库 `data/talent.db`，只写独立 campaign 目录。

## 关键产物

- Campaign：`data/campaigns/boss-app-all-match-50-20260531/`
- 结构化候选人：`data/campaigns/boss-app-all-match-50-20260531/structured/candidates.jsonl`
- 联系决策：`data/campaigns/boss-app-all-match-50-20260531/structured/contact-decisions.jsonl`
- 原始列表卡片：`data/campaigns/boss-app-all-match-50-20260531/raw/list-cards.jsonl`
- 原始详情页：`data/campaigns/boss-app-all-match-50-20260531/raw/detail-pages.jsonl`
- 摘要报告：`data/campaigns/boss-app-all-match-50-20260531/reports/sourcing-summary.md` / `.json`
- 恢复入口：`data/campaigns/boss-app-all-match-50-20260531/state/continuation-plan.json`
...
- Would contact：`50`
- Live-test 真实沟通：`0`
- 真实姓名补全：`0`，原因是本轮没有真实沟通，BOSS 列表/详情只提供展示名。
- 详情淘汰：`0`
- 人工复核清单：`0`
- Continuation 状态：`completed_after_50_dry_run`

## 执行观察

- BOSS App 返回列表后存在跳回已处理区段的情况，因此执行中以本地 append-only 候选人记录识别重复卡片，遇到已采集人选继续向下滚动寻找未处理卡片。
- 第 47-50 位补采人选为：董先生（阿里巴巴/大模型算法）、Leo Li（Meta/技术总监）、李先生（腾讯/测试开发）、王先生（腾讯/技术总监）。
- 部分人选虽然个人描述存在岗位偏好，例如只看 CTO 岗位、测试经理/测试总监，但本轮用户明确“全部人选都匹配”，因此仍记录为 `contact`，并把偏好作为后续真实沟通前的复核风险。

## 验证

- `.venv/bin/python -m scripts.boss_app_sourcing summarize --campaign-root data/campaigns/boss-app-all-match-50-20260531` -> `candidate_count=50`、`detail_count=50`、`would_contact_count=50`、`live_contact_count=0`
- `.venv/bin/python -m pytest tests/test_boss_app_sourcing.py -q` -> `36 passed`
