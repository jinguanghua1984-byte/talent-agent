...
- 若采用外部触达执行器，Codex 只能写 `structured/approved-contact-queue.jsonl` 和 `state/current-contact-intent.json` 并展示命令，不点击真实触达按钮；不得自行通过 shell、Computer Use 或任何运行时启动带 `--execute` 的外部执行器。真实点击只能由用户在独立终端显式启动独立 CLI，例如 `scripts.boss_contact_executor contact-current --execute`。
