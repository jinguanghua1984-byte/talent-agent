...
输入 A: `--candidates batch:<batch-id>`
  1. 读取批次: `python -m scripts.data_manager batch get <batch-id>`
  2. 获取候选人列表: `python -m scripts.data_manager batch candidates <batch-id> --filter "score>0"`
  3. 展示批次摘要表格（name, company, title, score）
  4. 用户确认或筛选

输入 B: `--candidates "company=阿里巴巴"` 或 `--candidates "enrichment=raw"`
  1. 列出候选人: `python -m scripts.data_manager candidate list`
...
输入 A: `--jd <jd-id>`
  ```bash
  python -m scripts.data_manager jd get <jd-id>
  ```
...
FOR EACH 用户选中:
  ...
  2. 不存在 → 创建临时 JSON → `python -m scripts.data_manager candidate create <tmp-file>`
