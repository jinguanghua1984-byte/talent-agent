...
只有前四个层级命中且综合分数 `>=95`，才允许写入 `auto_bound`。`name_company_fallback` 不得自动绑定；fallback 命中、候选过多、第二名分差过小、综合分 70-94、字段证据冲突或关键证据缺失时，状态写为 `pending_confirmation`。无结果或低分不匹配时写 `no_match`；明确排除时写 `rejected`。
