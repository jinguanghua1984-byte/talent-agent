...
- **`shell.run`** — 调用 `python -m scripts.data_manager` 读写数据
- **`file.read`** — 读取参考文件和候选人 JSON
- **`file.write`** — 写入评估报告 md

## 参考

- 评估标准：`agents/workflows/screen/references/eval-criteria.md`
...
python -m scripts.data_manager candidate list

# 3. 加载客户偏好（按 JD 中的 company 字段查找）
python -m scripts.data_manager rules get <company>
```

读取 `agents/workflows/screen/references/eval-criteria.md` 获取评估标准。
...
```bash
# 1. 记录修正到客户偏好
python -m scripts.data_manager rules add-correction <company> '<json>'
...
# 2. 写入 screening 结果
python -m scripts.data_manager screen create <jd-id> <cand-id> <adjusted-score>

# 3. 如果有红旗或差距，更新 screening
# 先写入临时 JSON 文件，再 update
python -m scripts.data_manager screen update <jd-id> <cand-id> <update-file.json>
```
