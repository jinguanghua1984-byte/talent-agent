...
### Step 1: 读取数据
1. 读取JD：`python -m scripts.data_manager jd get <jd-id>`
2. 读取筛选结果：`python -m scripts.data_manager screen list <jd-id>`
3. 对每个 screened/reported 的候选人，读取详细信息：
   `python -m scripts.data_manager candidate get <candidate-id>`
...
### Step 3: 按模板生成报告
按 `agents/workflows/report/references/report-template.md` 模板生成推荐报告：
1. JD 概要（从 JD 数据提取）
2. 每个候选人：
   - 画像摘要
   - 匹配亮点（从 screen 结果读取）
...
3. 更新所有包含候选人的 screen status 为 "reported"：
   对每个候选人执行 `python -m scripts.data_manager screen update <jd-id> <candidate-id> <tmpfile>`
